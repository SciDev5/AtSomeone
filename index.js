...
client.on("message",async message => {
    ...
    if (message.mentions.users.find((_user,id) => id === client.user.id)) {
        ...
        var members = message.guild.members.cache || await message.guild.members.fetch();
        await message.channel.send(message.content.replace(new RegExp(`<@!${client.user.id}>`,"g"),()=>`<@!${members.random().id}>`));
    }
